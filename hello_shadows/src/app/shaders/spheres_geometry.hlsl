...
void    main(point vertex_input input[1], inout TriangleStream<interpolated_value> outputStream )
{
	...
	vert[0]					= input[0].m_position_os.xyz - right - up; // Get bottom left vertex
	vert[1]					= input[0].m_position_os.xyz + right - up; // Get bottom right vertex
	vert[2]					= input[0].m_position_os.xyz - right + up; // Get top left vertex
	vert[3]					= input[0].m_position_os.xyz + right + up; // Get top right vertex
