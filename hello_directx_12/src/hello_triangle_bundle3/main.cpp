...
static winrt::com_ptr<ID3D12Resource1> CreateUploadTexture(ID3D12Device1* device, uint64_t size, D3D12_RESOURCE_STATES state)
{
    D3D12_RESOURCE_DESC d = {};
    d.Alignment = 0;
    d.DepthOrArraySize = 1;
...
static winrt::com_ptr<ID3D12Resource1> CreateTexture(ID3D12Device1* device, uint32_t width, uint32_t height, D3D12_RESOURCE_STATES state)
{
    D3D12_RESOURCE_DESC d = {};
    d.Alignment = 0;
    d.DepthOrArraySize = 1;
...
class MyViewProvider : public winrt::implements<MyViewProvider, IFrameworkView, IFrameworkViewSource>
{
    ...
    void Load(winrt::hstring h)
    {
        ...
        m_cube_texture = CreateTexture(m_device.get(), 32, 32, D3D12_RESOURCE_STATE_COPY_DEST);
        ...
        winrt::com_ptr <ID3D12Resource1 >			upload_cube_texture = CreateUploadTexture(m_device.get(), GetRequiredIntermediateSize(m_cube_texture.get(), 0, 1), D3D12_RESOURCE_STATE_GENERIC_READ);
        ...
        std::vector<uint32_t> pixels(1 * 32 * 32, 0);
        ...
        for (uint32_t i = 0; i < 1; ++i)
        {
            ...
            for (auto k = 0; k < 32 * 2; ++k)
            {
                *p++ = colors[1];
            }

            for (auto k = 32 * 2; k < 32 * 32; ++k)
            {
                *p++ = colors[0];
            }
        ...
        UpdateSubresources(upload_list.get(), m_cube_texture.get(), upload_cube_texture.get(), 0, 0, 1U, &d[0]);
	...
	void GenerateBundle()
	{
		...
		//This may involve drawing a character.
		{
...

            {
                ...
                m_bundle_command_list->SetGraphicsRootDescriptorTable(1, gpu(0));
            }
