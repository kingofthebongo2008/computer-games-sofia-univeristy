...
static inline XMFLOAT4X4 AssimpMat4ToMatrix(const aiMatrix4x4& mat)
{
    XMMATRIX result = XMLoadFloat4x4(reinterpret_cast<const XMFLOAT4X4*>(&mat));
    XMMATRIX result1 = XMMatrixTranspose(result);
    XMFLOAT4X4 s;
    XMStoreFloat4x4(&s, result1);
    return s;
}
...
static IModelImporter::ModelSourceType ExtractSourceTypeFromFileName(const char* input)
{
	...
	//extension = std::loextension.to_lower();
...
static void CreateGeom(const aiMesh* mesh, const char* name, AssimpImporter::Mesh* pMesh)
{
	...
	if (mesh->HasBones())
	{
		...
		for (uint32_t i = 0; i < mesh->mNumBones; ++i)
		{
            ...
            XMMATRIX        offsetMat  = XMLoadFloat4x4(reinterpret_cast<XMFLOAT4X4*>(&mat));
            XMMATRIX        transposed = XMMatrixTranspose(offsetMat);
            XMFLOAT4X4      store;
            
            XMStoreFloat4x4(&store, transposed);

			pMesh->mBones[i] = { bone->mName.C_Str(), store };
		}
	}
...
static void CollectNodes(const aiScene* pScene, AssimpImporter::Model* pModel)
{
	...
	if (pModel->mSourceType == IModelImporter::MODEL_SOURCE_TYPE_FBX)
	{
        ...
        {
            XMMATRIX m = XMMatrixScaling(0.01f, 0.01f, 0.01f) * XMLoadFloat4x4(&pModel->mRootNode.mChildren[i].mTransform);
            XMStoreFloat4x4(&pModel->mRootNode.mChildren[i].mTransform, m);
        }
	}
}
